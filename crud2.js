...
const { UV_FS_O_FILEMAP, RSA_NO_PADDING } = require('constants');
...
//사용자 추가 퀴리실행-함수형 변수
addUser = function(id, name, age, password, callback) {
    pool.getConnection(function(err,conn){
        if(err){
            if(conn) {
                conn.release();
        ...
        //html 파라미터 데이터를 insert 쿼리에 사용하기 위해서 객테로 만듦
        var dat = {id:id, name:name, age:age, password:password} //json데이터타입의 객테
        //SQL문 실행
